...
void rpy_callback(const geometry_msgs::PointStamped& rpy_msg)
{
  imu_data_qua = tf::createQuaternionMsgFromRollPitchYaw(rpy_msg.point.x/180*PI,rpy_msg.point.y/180*PI,rpy_msg.point.z/180*PI);
  // imu_data_yaw = tf::createQuaternionMsgFromYaw(rpy_msg.point.z/180*PI);
}

void pos_world_callback(const geometry_msgs::PointStamped& pos_world_msg)
{
  leg_pose_data.pose.pose.position  = pos_world_msg.point;
}
...
int main(int argc, char **argv)
{
  ...
  rpy_sub = nh.subscribe("rpy",1,rpy_callback);
  pos_world_sub = nh.subscribe("pos_world",1,pos_world_callback);
  ...
  while(ros::ok())
  {
    ...
    leg_odom_trans.transform.translation.x = leg_pose_data.pose.pose.position.x;
    leg_odom_trans.transform.translation.y = leg_pose_data.pose.pose.position.y;
    leg_odom_trans.transform.translation.z = leg_pose_data.pose.pose.position.z;
    leg_odom_trans.transform.rotation = imu_data_qua;
