...
if __name__ == "__main__":
    ...
    f_b = 0
    i = -1
    curtime = time.strftime(r"%m_%d_%H_%M", time.localtime())
    absPath = r"/home/sdu/dos_ws/src/chassis_dog/doc/"
    fn = absPath + f"vel_{curtime}.csv"
    fn2 = absPath + f"force_{curtime}.csv"
    fn3 = absPath + f"exp_vel_{curtime}.csv"

    # Open CSV files for writing
    with open(fn, "w", newline="") as vel_file, open(
        fn2, "w", newline=""
    ) as force_file, open(fn3, "w", newline="") as exp_vel_file:

        vel_writer = csv.writer(vel_file)
        force_writer = csv.writer(force_file)
        exp_vel_writer = csv.writer(exp_vel_file)

        # Write headers to CSV files
        vel_writer.writerow(["Time", "Velocity"])
        force_writer.writerow(["Time", "Force"])
        exp_vel_writer.writerow(["Time", "Exp_Velocity"])
        ...
        while True:
            ...
            # Log data to the lists
            vel_log.append(out_vel)
            force_log.append(force)
            exp_vel_log.append(exp_vel)

            # Write data to CSV files every iteration
            vel_writer.writerow([i * t, out_vel])
            force_writer.writerow([i * t, force])
            exp_vel_writer.writerow([i * t, exp_vel])
